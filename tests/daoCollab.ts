...
import { DaoCollab } from "../target/types/dao_collab";
import {ASSOCIATED_TOKEN_PROGRAM_ID,TOKEN_PROGRAM_ID,MINT_SIZE, createInitializeMintInstruction,createAssociatedTokenAccountInstruction,createTransferInstruction,createMintToInstruction, getAssociatedTokenAddress} from "@solana/spl-token"

describe("daoCollab", () => {
  // Configure the client to use the local cluster.
  anchor.setProvider(anchor.Provider.env());

  const program = anchor.workspace.DaoCollab as Program<DaoCollab>;
  ...
  const mintKey = anchor.web3.Keypair.generate();
    ...
    it("Should Create a Merchant Account",async() => {
      ...
      const [superAdmin, nonce] = await anchor.web3.PublicKey.findProgramAddress(
        [Buffer.from("super"),program.provider.wallet.publicKey.toBuffer()],
        program.programId
      );
      const tx = await program.rpc.createSuper({accounts:{
    ...
    it("Is initialized!", async () => {
      ...
      const tx = await program.rpc.mintKudosNft("DRPRK","DR","https://ipfs.io/ipfs/QmP7UWyKkcoH5TEg4bLjvMoHmdxJRhednBzGkHqbWUgGX4",1000,true,{
        accounts:{
          authority: program.provider.wallet.publicKey,
          superAccount:superAdmin,
